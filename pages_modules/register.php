...
//thejonyx-archivo con la config del modulo de referido
$get_ref = simplexml_load_file('engine/referido_settings.xml');
//thejonyx-archivo con la config del modulo de referido
...
function parse_inputs(){
    ...
    if (isset($_POST['create_account'])) {
        ...
        if ($err === true) {
			...
			//thejonyx-recibe de quien eres referido
			$referido_por  = safe_input($_POST['referido'], '');
			//thejonyx-recibe de quien eres referido
            if ($get_config->pers_id_active == '1') {
                    ...
                    if (check_mail($email) === true) {
                        ...
                        if ($core['config']['md5'] == '1') {
                            $make_me_acc = $core_db2->Execute("INSERT INTO MEMB_INFO (memb___id,memb__pwd,memb_name,sno__numb,bloc_code,ctl1_code,mail_chek,mail_addr,appl_days,modi_days,out__days,true_days,SecretQuestion,SecretAnswer,Country,Gender,confirmed,activation_id) VALUES (?,[dbo].[fn_md5](?,?),?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", array(
								...
								$nombre_referido = 'referido/user/'.$referido_por.'.tDB';
									if (!file_exists($nombre_referido)) {
    									$new_db = fopen('referido/user/'.$referido_por.'.tDB', "w+");
    									fwrite($new_db);
    									fclose($new_db);
										} 
								//abres el archivo y se guarda el referido y la fecha de registro
    							$db = fopen('referido/user/'.$referido_por.'.tDB', "a+");
    							fwrite($db, $userid . "¦" . date('d/m/Y') . "¦\n");
    							fclose($db);
								//thejonyx-cierras el archivo y se guarda el referido y la fecha de registro
...
<td align="left" class="iRg_inf"><input class="iRg_input" type="text" name="userid" id="userid" maxlength="10" value="' . $userid_post . '"></td>
...
//thejonyx-mostrar si en admin eligieron q se vea este bloque
if ($get_ref->f5 == '1') {
            echo '
<tr>
<td align="left" class="iRg_text"  style="padding-left: 24px;">Referido P:</td>
<td align="left" class="iRg_inf">
<input class="iRg_input" type="text" id="referido" name="referido" value="' . $_GET['ref'] . '" disabled>
<input type="hidden" name="referido" value="' . $_GET['ref'] . '">
</td>
<td align="left" ><span class="iRg_inf"><div id="c_uss">A sido Referido Por.</div></span></td>
</tr>';
}
...
elseif ($get_ref->f5 == '2'){
	...
	echo '
<tr>
<td align="left" class="iRg_text"  style="padding-left: 24px;">Referido:</td>
<td align="left" class="iRg_inf">
<select name="referido" class="iRg_input" id="referido" style="width: 164px;">
<option value="'.$_GET['ref'].'">'.$_GET['ref'].'</option>';
$query = mssql_query('SELECT [memb___id] FROM [dbo].[MEMB_INFO]');
				for ($i = 0; $i < mssql_num_rows($query); ++$i) {
				$hola = mssql_result($query, $i, 'memb___id');
						echo'<option value="'.$hola.'">'.$hola.'</option>';
    			}
echo '
</select>
<script src="referido/style/jquery.js" type="text/javascript"></script>
<script src="referido/style/chosen.jquery.js" type="text/javascript"></script>
<link rel="stylesheet" href="referido/style/chosen.css">
</td>
<td align="left" ><span class="iRg_inf"><div id="c_uss">A sido Referido Por.</div></span></td>
</tr>';
	...
	echo '<input type="hidden" name="referido" value="' . $_GET['ref'] . '">';
